/*
    ...
    following. Note that offsets are given by line number while in the binary
...
int main(int argc, char *argv[]) {
    ...
    // check the first bytes to ensure the correct file format
    int ident_matches = header->ident[0] == 0xDE && header->ident[1] == 'D' &&
                        header->ident[2] == 'I' && header->ident[3] == 'R';
    ...
    size_t offset = -1;
    int num_contacts = -1;
    // Iterate over the array checking for desired department code
